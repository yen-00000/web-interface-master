...
# 自动匹配浏览器配置
CHROME_DRIVER_PATH = webConfig.DRIVER_PATH + '\\chromedriver.exe'
FIREFOX_DRIVER_PATH = webConfig.DRIVER_PATH + '\\geckodriver.exe'
IE_DRIVER_PATH = webConfig.DRIVER_PATH + '\\IEDriverServer.exe'
...
class Browser(object):
    ...
    def __init__(self, browser_type='chrome'):
        ...
        # 设置显式等待时间
        self.wait = WebDriverWait(self.driver, webConfig.WebConfig.timeout)
        # 设置隐式等待时间
    ...
    def customizeDownload(self):
        ...
        if self._type in TYPES:
            if self._type == 'firefox':
                ...
                return webdriver.Firefox(executable_path=EXECUTABLE_PATH[self._type], firefox_profile=profile)
                ...
                return webdriver.Chrome(executable_path=EXECUTABLE_PATH[self._type],
    ...
    def findEleUntil(self, value, by=By.XPATH):
        ...
        return self.wait.until(EC.element_to_be_clickable((by, value)),
                               'Unable to locate element until timeout {"method":"%s","selector":"%s"}' % (by, value))
    ...
    def typeByCSS(self, css, value):
        """输入操作，css定位元素"""
        element = self.findEle(css, By.CSS_SELECTOR)
        self.clear(element)
        if value == 'username':
            value = configInfo.UserConfig.username
        if value == 'password':
            value = configInfo.UserConfig.password
        element.send_keys(value)
    ...
    def sliderVerify(self, imgCss='div#__Verification > div:nth-child(1)', blockCss='div#xy_img',
        ...
        while num < 10:  # 设置重试次数为10
            ...
            time.sleep(1)
            try:
                self.findEleUntil(imgCss, By.CSS_SELECTOR)  # 查看验证图片是否存在
            except Exception as e:
                print(e)
